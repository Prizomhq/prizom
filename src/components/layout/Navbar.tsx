              ...
              <div className="hidden lg:flex space-x-6 items-center">
                ...
                <Link href="/studio" className={`text-sm transition-colors flex items-center gap-1.5 hover:text-purple-600 ${pathname?.startsWith('/studio') || pathname?.startsWith('/create/studio') ? 'text-purple-600 font-bold' : 'text-zinc-500 font-medium'}`}>
                  ...
                  <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${
                    profile?.role === 'super_admin' ? 'bg-purple-100 text-purple-700' : 'bg-amber-100 text-amber-800 border border-amber-200/60'
                  }`}>
                    {profile?.role === 'super_admin' ? 'Studio' : 'Coming Soon'}
                  </span>
                </Link>
              </div>
            </div>
                ...
                {user ? (
                  <>
                    <Link 
                      href="/studio" 
                      onClick={() => setIsOpen(false)} 
                      className="px-4 py-3 rounded-xl text-base font-extrabold text-purple-600 hover:bg-purple-50 transition-colors flex items-center justify-between cursor-pointer"
                    >
                      ...
                      <span className="px-2 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px] font-black uppercase">
                        {profile?.role === 'super_admin' ? 'Studio' : 'Soon'}
                      </span>
                    </Link>
